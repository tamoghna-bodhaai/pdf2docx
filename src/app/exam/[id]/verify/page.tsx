    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-2xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
        <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-slate-200">
          <h1 className="font-bold text-slate-900 leading-tight">{exam.name} — {exam.subject}</h1>
          <p className="text-xs text-slate-500 mt-1">{exam.questionCount} questions • {exam.markingScheme ? `${formatScheme(exam.markingScheme)} • Max ${computeMax(exam.markingScheme)}` : `${exam.marksPerQuestion} marks each`}</p>

          {/* Marking Scheme Editor */}
          <div className="mt-4 border border-slate-200 rounded-xl p-3 bg-slate-50">
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs font-semibold text-slate-800">Marking Scheme</span>
              {!markingEdit ? <button onClick={()=>setMarkingEdit(true)} className="text-xs border border-slate-200 bg-white px-3 py-1.5 rounded-full font-medium hover:bg-slate-50">Edit</button> :
                <div className="flex gap-2"><button onClick={()=>setMarkingEdit(false)} className="text-xs border border-slate-200 bg-white px-3 py-1.5 rounded-full">Cancel</button><button onClick={saveMarkingScheme} disabled={markingSaving} className="text-xs bg-indigo-600 text-white px-3 py-1.5 rounded-full disabled:opacity-50">{markingSaving?"Saving...":"Save"}</button></div>}
            </div>
            {!markingEdit ? (
              <div className="mt-2 flex flex-wrap gap-1.5">
                {(exam.markingScheme||[{from:1,to:exam.questionCount,marks:exam.marksPerQuestion,negativeMarks:exam.negativeMarks}]).map((s:any,i:number)=>(
                  <span key={i} className="text-[11px] bg-white border border-slate-200 px-2 py-1 rounded-full font-medium">Q{s.from}-{s.to}: +{s.marks}/-{s.negativeMarks}</span>
                ))}
                <span className="text-[11px] text-slate-500 ml-1">Max {exam.markingScheme ? computeMax(exam.markingScheme): exam.questionCount*exam.marksPerQuestion} marks</span>
              </div>
            ) : (
              <div className="mt-3 space-y-2">
                {sections.map((sec, idx)=>(
                  <div key={idx} className="bg-white border border-slate-200 rounded-xl p-2 flex gap-2 items-end">
                    <div className="flex-1 grid grid-cols-4 gap-2">
                      <label className="space-y-1"><span className="text-[10px] text-slate-500">From</span><div className="bg-slate-100 border border-slate-200 rounded-lg px-2 py-1.5 text-xs text-center font-bold">{sec.from}</div></label>
                      <label className="space-y-1"><span className="text-[10px] text-slate-500">To</span><input type="number" min={sec.from} max={exam.questionCount} value={sec.to} onChange={e=>{
                      ...
                      }} className="w-full border border-slate-200 rounded-lg px-1 py-1.5 text-xs text-center" /></label>
                      <label className="space-y-1"><span className="text-[10px] text-slate-500">+Marks</span><input type="number" step="0.5" value={sec.marks} onChange={e=>{ const v=parseFloat(e.target.value)||0; setSections(prev=>{ const c=[...prev.map(x=>({...x}))]; c[idx].marks=v; return c;});}} className="w-full border border-slate-200 rounded-lg px-1 py-1.5 text-xs text-center" /></label>
                      <label className="space-y-1"><span className="text-[10px] text-slate-500">-Neg</span><input type="number" step="0.25" value={sec.negativeMarks} onChange={e=>{ const v=parseFloat(e.target.value)||0; setSections(prev=>{ const c=[...prev.map(x=>({...x}))]; c[idx].negativeMarks=v; return c;});}} className="w-full border border-slate-200 rounded-lg px-1 py-1.5 text-xs text-center" /></label>
                    </div>
                    ...
                    }} className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 disabled:opacity-30 bg-white">✕</button>
                  </div>
                ))}
                <button type="button" onClick={()=>{
                  ...
                  setSections(copy);
                }} className="text-xs bg-white border border-indigo-200 text-indigo-700 px-3 py-1.5 rounded-full font-medium hover:bg-indigo-50">+ Add Section</button>
                <p className="text-[11px] text-slate-500">Must fully cover 1..{exam.questionCount} contiguously. Saving will re-grade all already-graded submissions.</p>
              </div>
            )}
          </div>
          ...
          <h2 className="mt-6 font-semibold text-sm text-slate-900">Answer Key</h2>
          <div className="mt-3 grid grid-cols-3 xs:grid-cols-4 sm:grid-cols-5 gap-2 max-h-[45vh] sm:max-h-[50vh] overflow-y-auto pr-1">
            {Array.from({length: exam.questionCount}, (_,i)=>{
              const q = String(i+1);
              return (
                <label key={q} className="border border-slate-200 rounded-xl p-2 bg-slate-50 flex flex-col gap-1">
                  <span className="text-xs font-medium text-slate-600">Q{q}</span>
                  <select value={keyJson[q]||"A"} onChange={e=>setKeyJson({...keyJson, [q]: e.target.value})} className="border border-slate-200 rounded-lg px-2 py-2 text-sm font-bold bg-white min-h-[36px]">
          ...
          <div className="mt-4">
            <span className="text-xs font-medium text-slate-700">Bulk edit (comma-separated)</span>
            <textarea
              ...
              rows={2}
              className="w-full mt-1.5 border border-slate-200 rounded-xl px-3 py-2.5 text-xs font-mono bg-white focus:ring-2 focus:ring-indigo-500 outline-none"
              placeholder="B,D,A,C,..."
            />
          </div>
          ...
          <div className="mt-6 flex gap-3">
            <button onClick={save} disabled={saving} className="flex-1 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 disabled:opacity-50 text-white py-3.5 rounded-xl font-medium min-h-[48px] transition">
              {saving? "Saving...":"Confirm & Start Scanning"}
            </button>
          </div>
          <Link href={`/exam/${id}/scan`} className="block text-center text-xs text-slate-500 mt-3 hover:underline hover:text-slate-700 py-2">Skip to scanning (keep current key)</Link>
        </div>
      </main>
    </div>
  );
