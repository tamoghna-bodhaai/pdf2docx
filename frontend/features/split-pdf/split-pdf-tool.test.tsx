...
describe("split range", () => {
  ...
  it("keeps merge and extraction controls in Setup and cancels an unfinished drag", async () => {
    ...
    const setup = screen.getByRole("complementary", { name: "Tool workspace" });
    expect(setup).toContainElement(screen.getByRole("checkbox", { name: /Merge ranges into one PDF/ }));
    expect(setup).toContainElement(screen.getByRole("button", { name: "Extract pages" }));
